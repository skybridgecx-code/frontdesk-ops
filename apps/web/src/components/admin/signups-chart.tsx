...
export function SignupsChart() {
  ...
  const [data, setData] = useState<SignupsPoint[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        const response = await fetch(`/api/admin/metrics/signups-over-time?period=${period}`, {
          cache: 'no-store',
          signal: controller.signal
        });

        const payload = (await response.json()) as SignupsOverTimeResponse | { error?: string };
  ...
  return (
    <div className="rounded-xl border border-gray-800 bg-gray-900 p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h3 className="font-semibold text-white">Signups Over Time</h3>
      ...
      {loading ? <p className="text-sm text-gray-400">Loading signup chart...</p> : null}
      {!loading && error ? <p className="text-sm text-red-400">{error}</p> : null}

      {!loading && !error ? (
        <div className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 6, right: 10, left: -14, bottom: 0 }}>
